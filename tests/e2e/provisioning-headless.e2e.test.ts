...
function buildComposeArgs(projectName: string, ...args: string[]) {
  return ["compose", "-f", e2eComposeFile, "-p", projectName, ...args];
}
...
async function getWalletPersistenceState(
  databaseUrl: string,
  walletId: string,
) {
  const pool = new Pool({
...
async function runCliFailure(args: string[], env: NodeJS.ProcessEnv) {
  try {
    await runCommand({
      command: "node",
      args: [cliEntry, ...args],
...
describe("headless provisioning e2e", () => {
  it("provisions a wallet and executes co-signed runtime operations through the real local stack", async () => {
    ...
    let x402Server: Awaited<
      ReturnType<typeof startX402ExactEip3009Server>
    > | null = null;
    ...
    try {
      ...
      const fundingAmount =
        BigInt(published.publishedWallet.funding.minimumRequiredWei) +
        parseEther("0.05");
      ...
      const fundingHash = await fundingClient.sendTransaction({
        to: published.publishedWallet
          .counterfactualWalletAddress as `0x${string}`,
        value: fundingAmount,
      });
      ...
      const walletRequest = getWalletRequestResponseSchema.parse(
        await (await fetch(`${backendUrl}/v1/wallets/${walletId}`)).json(),
      );
      ...
      expect(walletRequest.walletContext?.walletAddress).toBe(
        localWallet.walletAddress,
      );
      expect(walletRequest.walletContext?.agentAddress).toBe(
        localWallet.agentAddress,
      );
      expect(walletRequest.walletContext?.backendAddress).toBe(
        localWallet.backendAddress,
      );
      expect(localWallet.walletAddress).toBe(
        walletRequest.walletContext?.walletAddress,
      );
      expect(localWallet.ownerPublicArtifacts).toEqual(
      ...
      expect(walletRequest.deployment.status).toMatch(
        /^(undeployed|deployed)$/,
      );
      ...
      const persistedAfterDeniedTypedData = await getWalletPersistenceState(
        databaseUrl,
        walletId,
      );
      ...
      const allowedCallReceipt =
        await anvilPublicClient.waitForTransactionReceipt({
          hash: allowedCallResult.transactionHash as `0x${string}`,
        });
      const persistedAfterAllowedCall = await getWalletPersistenceState(
        databaseUrl,
        walletId,
      );
      ...
      const persistedAfterDeniedCall = await getWalletPersistenceState(
        databaseUrl,
        walletId,
      );
      ...
      const x402FetchResult = await runCliJson(
        ["x402-fetch", walletId, x402Server.protectedUrl],
        cliEnv,
      );
      ...
      expect(paymentRequired.accepts[0]?.amount).toBe(
        x402Server.paymentAmount.toString(),
      );
      expect(paymentRequired.accepts[0]?.payTo).toBe(
        x402Server.merchantAddress,
      );
      ...
      expect(settlementResponse.amount).toBe(
        x402Server.paymentAmount.toString(),
      );
      expect(merchantUsdcAfter - merchantUsdcBefore).toBe(
        x402Server.paymentAmount,
      );
      ...
      const persistedAfterX402 = await getWalletPersistenceState(
        databaseUrl,
        walletId,
      );
      ...
      const persistedAfterDeniedX402 = await getWalletPersistenceState(
        databaseUrl,
        walletId,
      );

      expect(deniedX402Error).toContain("Backend signer request failed");
      expect(deniedX402Error).toContain(
        "configured USDC budget has been exceeded",
      );
