...
- Product design: `docs/superpowers/specs/2026-03-25-conduit-wallet-design.md`
- Technical spec: `docs/superpowers/specs/2026-03-25-conduit-wallet-technical-spec.md`
- Handoff brief: `docs/superpowers/tracking/2026-03-25-conduit-wallet-handoff.md`
...
- Working implementation configuration:
  ...
  - CLI local env usage is limited to `CONDUIT_BACKEND_URL` as an optional convenience override
...
2. `pnpm build`
3. `pnpm test`
4. `pnpm --filter @conduit/cli exec node dist/index.js --help`
5. `DATABASE_URL=... pnpm --filter @conduit/backend dev` then `curl http://127.0.0.1:3000/health`
6. `pnpm --filter @conduit/frontend dev` then `curl http://127.0.0.1:5173`
7. `pnpm --filter @conduit/cli exec node dist/index.js create ...`
8. `pnpm --filter @conduit/cli exec node dist/index.js status <wallet-id> ...`
...
14. `pnpm --filter @conduit/shared build`
15. `pnpm --filter @conduit/backend build`
16. `pnpm --filter @conduit/cli build`
17. `pnpm --filter @conduit/frontend build`
...
21. CLI entrypoint verified to read `CONDUIT_BACKEND_URL` from a temporary root `.env.local`
22. frontend build verified with `CONDUIT_PASSKEY_SERVER_URL` and chain RPC settings coming only from a temporary root `.env.local`
...
33. live `conduit-wallet call` succeeded on Base Sepolia against USDC `approve(address,uint256)` for wallet `wal_c700ae7d9b64436c9dcac2456e4172cc`
...
- backend proxy transport was re-validated live: `conduit-wallet call` succeeded against Base Sepolia through `/v1/chains/84532/rpc` and `/v1/chains/84532/bundler`, returning transaction hash `0xe2203902730242b8d18da7f69509c702f2dfe4d737ca98bd26e7babe257bd01e`
...
- decide whether `CONDUIT_PUBLIC_RPC_URL_<chainId>` remains the stable env contract for both frontend and backend runtime
...
- The execution plan for the remaining V1 delta lives in `docs/superpowers/plans/2026-03-25-conduit-wallet-v1-completion.md`.
- The deleted file `docs/superpowers/plans/2026-03-25-conduit-wallet-v1.md` existed before this bootstrap work and was intentionally left untouched.
